...
// export const isForger = (element: FieldElement): element is HTMLInputElement =>
//   element.;

type SlotProps = {
  children?: React.ReactNode;
  style?: StyleProp<any>;
  disableChildrenStyle?: boolean;
};
...
export function Slot({ children, ...props }: SlotProps) {
  ...
  if (React.isValidElement(children)) {
    const customChildStyle = props?.disableChildrenStyle ? [] : children?.props?.style ?? [];
